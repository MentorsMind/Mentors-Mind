...
export function RoleSelection() {
  const [selectedRole, setSelectedRole] = useState<"mentor" | "learner" | null>(
    null,
  );
          ...
          <label className="group relative cursor-pointer">
            ...
            <div className="relative flex items-center p-5 rounded-xl border-2 border-transparent bg-white dark:bg-[#1c2e24] transition-all duration-300 ease-in-out peer-checked:border-primary peer-checked:bg-primary/5 dark:peer-checked:bg-primary/10 peer-checked:shadow-[0_0_20px_-5px_rgba(16,162,75,0.3)] hover:bg-gray-50 dark:hover:bg-[#25382e]">
              ...
              <div className="flex flex-col flex-1">
                <span className="text-lg font-bold text-slate-900 dark:text-white mb-1 group-hover:text-primary transition-colors">
                  I am a Mentor
                </span>
                <span className="text-sm font-medium text-slate-500 dark:text-[#9db8a8] leading-snug">
                  Share knowledge & guide startups
                </span>
              </div>
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ml-2 ${selectedRole === "mentor" ? "border-primary bg-primary" : "border-gray-300 dark:border-gray-600"}`}
              >
                <Check
                  className={`w-4 h-4 text-white transition-opacity ${selectedRole === "mentor" ? "opacity-100" : "opacity-0"}`}
                />
              </div>
            </div>
            {/* Category Selection for Mentor */}
            <div
              className={`overflow-hidden transition-all duration-300 ease-out ${selectedRole === "mentor" ? "max-h-40 opacity-100 mt-4" : "max-h-0 opacity-0 mt-0"}`}
            >
              <p className="text-sm font-medium text-slate-500 dark:text-gray-400 mb-2 ml-1">
          ...
          <label className="group relative cursor-pointer">
            ...
            <div className="relative flex items-center p-5 rounded-xl border-2 border-transparent bg-white dark:bg-[#1c2e24] transition-all duration-300 ease-in-out peer-checked:border-primary peer-checked:bg-primary/5 dark:peer-checked:bg-primary/10 peer-checked:shadow-[0_0_20px_-5px_rgba(16,162,75,0.3)] hover:bg-gray-50 dark:hover:bg-[#25382e]">
              ...
              <div className="flex flex-col flex-1">
                ...
                <span className="text-sm font-medium text-slate-500 dark:text-[#9db8a8] leading-snug">
                  Gain skills & find guidance
                </span>
              </div>
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ml-2 ${selectedRole === "learner" ? "border-primary bg-primary" : "border-gray-300 dark:border-gray-600"}`}
              >
                <Check
                  className={`w-4 h-4 text-white transition-opacity ${selectedRole === "learner" ? "opacity-100" : "opacity-0"}`}
                />
              </div>
            </div>
          </label>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background-light dark:from-background-dark to-transparent max-w-md mx-auto">
        <button
          ...
          disabled={
            !selectedRole ||
            isSubmitting ||
            (selectedRole === "mentor" && !selectedCategory)
          }
