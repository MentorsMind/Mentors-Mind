...
export function MentorDashboard() {
  ...
  const [editModalTab, setEditModalTab] = useState<"details" | "expertise">(
    "details",
  );

  const firstName = user?.name ? user.name.split(" ")[0] : "Mentor";
  const userImage =
    user?.image || "https://api.dicebear.com/7.x/avataaars/svg?seed=Mentor";
            ...
            <section>
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-bold">
                    {stats.requests.length} new
                  </span>
                </div>
                <button className="text-sm font-semibold text-primary hover:text-green-700 transition-colors">
                  See all
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {stats.requests.map((req) => (
                  <div
                    key={req.id}
                    className="group relative flex flex-col bg-white dark:bg-[#1a2e22] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all overflow-hidden"
                  >
                    ...
                    <div className="p-5 pt-8 flex flex-col gap-3">
                      <div>
                        <div className="flex flex-wrap gap-2 mb-2">
                          {req.tags.map((tag) => (
                            <span
                              key={tag}
                              className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 uppercase tracking-wide border border-gray-200 dark:border-white/5"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      <div className="grid grid-cols-2 gap-3 mt-2">
                        <button className="py-2.5 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-600 dark:text-gray-300 font-semibold text-sm hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                          Decline
                        </button>
                        <button className="py-2.5 rounded-xl bg-primary text-white font-semibold text-sm hover:bg-green-600 shadow-lg shadow-green-900/20 transition-all active:scale-[0.98]">
                          Accept
                        </button>
                      </div>
                    </div>
            ...
            <section>
              ...
              <div className="flex flex-col gap-4">
                {stats.questions.map((q) => (
                  <div
                    key={q.id}
                    className="p-4 rounded-2xl bg-white dark:bg-[#1a2e22] border border-gray-100 dark:border-gray-800 shadow-sm hover:border-primary/30 transition-colors group cursor-pointer"
                  >
                    <div className="flex items-start gap-3">
                      <div
                        className="size-10 rounded-full bg-cover bg-center shrink-0 ring-2 ring-gray-50 dark:ring-white/5"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium text-gray-600 dark:text-gray-300 leading-snug mb-3 line-clamp-2">
                          {q.text}
                        </p>
                        <div className="flex items-center justify-between">
                          <div className="flex gap-1.5">
                            {q.tags.map((tag) => (
                              <span
                                key={tag}
                                className="text-[10px] font-medium text-gray-500 bg-gray-100 dark:bg-white/5 px-1.5 py-0.5 rounded"
                              >
                                #{tag}
                              </span>
                            ))}
                  ...
                  <div className="bg-white/60 dark:bg-black/20 backdrop-blur-md p-4 rounded-2xl border border-white/50 dark:border-white/5 shadow-sm">
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        ...
                        style={{
                          backgroundImage: `url('${nextSession.learnerImage}')`,
                        }}
                      ></div>
                      <div>
                        <p className="font-bold text-gray-900 dark:text-white">
                          {nextSession.learnerName}
                        </p>
                        <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 mt-0.5">
                          {nextSession.topic.substring(0, 15)}...
                        </span>
                      </div>
                    </div>
